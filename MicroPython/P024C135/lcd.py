...
class LCD:
    ...
    def init(self):
        self.hard_reset()
        # ************* 开始初始化序列 **********
        self.LCD_WR_REG(0x11)  # Sleep out（退出睡眠模式）
        time.sleep_ms(120)     # 延时 120ms（等待屏幕唤醒）
        # ************* 开始初始化序列 **********

        # 寄存器 0xCF 配置
        self.LCD_WR_REG(0xCF)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0xD9)
        self.LCD_WR_DATA8(0X30)

        # 寄存器 0xED 配置
        self.LCD_WR_REG(0xED)
        self.LCD_WR_DATA8(0x64)
        self.LCD_WR_DATA8(0x03)
        self.LCD_WR_DATA8(0X12)
        self.LCD_WR_DATA8(0X81)

        # 寄存器 0xE8 配置
        self.LCD_WR_REG(0xE8)
        self.LCD_WR_DATA8(0x85)
        self.LCD_WR_DATA8(0x10)
        self.LCD_WR_DATA8(0x78)

        # 寄存器 0xCB 配置
        self.LCD_WR_REG(0xCB)
        self.LCD_WR_DATA8(0x39)
        self.LCD_WR_DATA8(0x2C)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x34)
        ...
        self.LCD_WR_DATA8(0x21)  # VRH[5:0] 配置

        # 寄存器 0xC1 配置（电源控制）
        self.LCD_WR_REG(0xC1)
        self.LCD_WR_DATA8(0x12)  # SAP[2:0]; BT[3:0] 配置

        # 寄存器 0xC5 配置（VCM 控制）
        self.LCD_WR_REG(0xC5)
        self.LCD_WR_DATA8(0x32)
        self.LCD_WR_DATA8(0x3C)

        # 寄存器 0xC7 配置（VCM 控制2）
        self.LCD_WR_REG(0xC7)
        self.LCD_WR_DATA8(0XC1)

        # 寄存器 0x36 配置（内存访问控制：屏幕旋转）
        self.LCD_WR_REG(0x36)
        if self.rotation == 0:
            self.LCD_WR_DATA8(0x08)
        elif self.rotation == 1:
            self.LCD_WR_DATA8(0xC8)
        elif self.rotation == 2:
            self.LCD_WR_DATA8(0x78)
        else:
            self.LCD_WR_DATA8(0xA8)

        # 寄存器 0x3A 配置（像素格式：16位 RGB565）
        self.LCD_WR_REG(0x3A)
        self.LCD_WR_DATA8(0x55)  # 0x55 = 16位模式（RGB565）

        # 寄存器 0xB1 配置（帧速率控制）
        self.LCD_WR_REG(0xB1)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x18)

        # 寄存器 0xB6 配置（显示功能控制）
        self.LCD_WR_REG(0xB6)
        self.LCD_WR_DATA8(0x0A)
        self.LCD_WR_DATA8(0xA2)

        # 寄存器 0xF2 配置（3Gamma 功能禁用）
        self.LCD_WR_REG(0xF2)
        self.LCD_WR_DATA8(0x00)

        # 寄存器 0x26 配置（Gamma 曲线选择）
        self.LCD_WR_REG(0x26)
        self.LCD_WR_DATA8(0x01)

        # 寄存器 0xE0 配置（正 Gamma 校正）
        ...
        self.LCD_WR_DATA8(0x50)
        self.LCD_WR_DATA8(0XBA)
        self.LCD_WR_DATA8(0x44)
        self.LCD_WR_DATA8(0x09)
        self.LCD_WR_DATA8(0x14)
        self.LCD_WR_DATA8(0x05)
        self.LCD_WR_DATA8(0x23)
        self.LCD_WR_DATA8(0x21)
        self.LCD_WR_DATA8(0x00)

        # 寄存器 0xE1 配置（负 Gamma 校正）
        self.LCD_WR_REG(0xE1)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x19)
        self.LCD_WR_DATA8(0x19)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x12)
        self.LCD_WR_DATA8(0x07)
        self.LCD_WR_DATA8(0x2D)
        self.LCD_WR_DATA8(0x28)
        self.LCD_WR_DATA8(0x3F)
        self.LCD_WR_DATA8(0x02)
        self.LCD_WR_DATA8(0x0A)
        self.LCD_WR_DATA8(0x08)
