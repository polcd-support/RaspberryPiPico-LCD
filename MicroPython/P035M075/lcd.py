...
class LCD:
    ...
    def init(self):
        ...
        self.LCD_WR_DATA8(0x0f)
        self.LCD_WR_DATA8(0x0D)
        self.LCD_WR_DATA8(0x1B)
        self.LCD_WR_DATA8(0x0A)
        self.LCD_WR_DATA8(0x3c)
        self.LCD_WR_DATA8(0x78)
        self.LCD_WR_DATA8(0x4A)
        self.LCD_WR_DATA8(0x07)
        self.LCD_WR_DATA8(0x0E)
        self.LCD_WR_DATA8(0x09)
        self.LCD_WR_DATA8(0x1B)
        self.LCD_WR_DATA8(0x1e)
        self.LCD_WR_DATA8(0x0f)

        # -------------------------- 负 Gamma 校正（0xE1 寄存器）--------------------------
        self.LCD_WR_REG(0xE1)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x22)
        self.LCD_WR_DATA8(0x24)
        self.LCD_WR_DATA8(0x06)
        self.LCD_WR_DATA8(0x12)
        self.LCD_WR_DATA8(0x07)
        self.LCD_WR_DATA8(0x36)
        self.LCD_WR_DATA8(0x47)
        self.LCD_WR_DATA8(0x47)
        self.LCD_WR_DATA8(0x06)
        self.LCD_WR_DATA8(0x0a)
        self.LCD_WR_DATA8(0x07)
        self.LCD_WR_DATA8(0x30)
        self.LCD_WR_DATA8(0x37)
        self.LCD_WR_DATA8(0x0f)

        # -------------------------- 电源控制 1（0xC0 寄存器）--------------------------
        self.LCD_WR_REG(0xC0)
        self.LCD_WR_DATA8(0x10)
        self.LCD_WR_DATA8(0x10)

        # -------------------------- 电源控制 2（0xC1 寄存器）--------------------------
        self.LCD_WR_REG(0xC1)
        self.LCD_WR_DATA8(0x41)

        # -------------------------- VCM 控制（0xC5 寄存器）--------------------------
        self.LCD_WR_REG(0xC5)
        self.LCD_WR_DATA8(0x00)
        self.LCD_WR_DATA8(0x22)
        self.LCD_WR_DATA8(0x80)

        # -------------------------- 内存访问控制（0x36 寄存器，屏幕旋转）--------------------------
        self.LCD_WR_REG(0x36)  # Memory Access Control
        if self.rotation == 0:
            self.LCD_WR_DATA8(0x48)
        elif self.rotation == 1:
            self.LCD_WR_DATA8(0x88)
        elif self.rotation == 2:
            self.LCD_WR_DATA8(0x28)
        else:
            self.LCD_WR_DATA8(0xE8)

        # -------------------------- 接口模式控制（0x3A 寄存器，ILI9486 适配）--------------------------
        self.LCD_WR_REG(0x3A)  # Interface Mode Control
        self.LCD_WR_DATA8(0x66)  # 此处为 ILI9486 专用配置（注意：非通用值）

        # -------------------------- 显示模式控制（0xB0 寄存器）--------------------------
        self.LCD_WR_REG(0xB0)
        self.LCD_WR_DATA8(0x00)

        # -------------------------- 帧率控制（0xB1 寄存器，70Hz）--------------------------
        self.LCD_WR_REG(0xB1)
        self.LCD_WR_DATA8(0xB0)
        self.LCD_WR_DATA8(0x11)

        # -------------------------- 显示时序控制（0xB4 寄存器）--------------------------
        self.LCD_WR_REG(0xB4)
        self.LCD_WR_DATA8(0x02)

        # -------------------------- RGB/MCU 接口控制（0xB6 寄存器）--------------------------
        self.LCD_WR_REG(0xB6)  # RGB/MCU Interface Control
        self.LCD_WR_DATA8(0x02)
        self.LCD_WR_DATA8(0x02)

        # -------------------------- 电源控制 3（0xB7 寄存器）--------------------------
        self.LCD_WR_REG(0xB7)
        self.LCD_WR_DATA8(0xC6)

        # -------------------------- 增强控制（0xE9 寄存器）--------------------------
        self.LCD_WR_REG(0xE9)
        self.LCD_WR_DATA8(0x00)

        # -------------------------- 功能控制（0xF7 寄存器）--------------------------
        self.LCD_WR_REG(0xF7)
        self.LCD_WR_DATA8(0xA9)
        self.LCD_WR_DATA8(0x51)
        self.LCD_WR_DATA8(0x2C)
        self.LCD_WR_DATA8(0x82)

        # -------------------------- 退出睡眠模式（0x11 寄存器）--------------------------
        self.LCD_WR_REG(0x11)
        time.sleep_ms(120)  # 等待 120ms 确保屏幕唤醒
    ...
    def fill_color_buffer(self, color: int, length: int):
        ...
        chunks, rest = divmod(length, MAX_BUFFER_SIZE * 3)
        
        # 将16位颜色值转换为大端序2字节数据
        # pixel = struct.pack('>H', color)
        # 生成256像素的缓冲区（256 * 2 = 512字节）
        buffer = color * MAX_BUFFER_SIZE  # 结果为字节数组
    ...
    def lcd_Fill(self,xsta, ysta, xend, yend, color):
        ...
        g = (color >> 3) & 0xFC
        b = (color << 3) & 0xFF 
        color_buffer = bytes([r, g, b])  
        
        #color_buffer = bytes([(color >> 8) & 0xF8,(color >> 3) & 0xFC,(color << 3)])
        self.cs(0)
        self.dc(1)
        self.fill_color_buffer(color_buffer,width*height*3)
    ...
    def LCD_DrawPoint(self,x,y,color):
        ...
        self.spi.write(color_buffer)  # 发送像素颜色数据
    ...
    def gui_draw_hline_batch(self, x0, y0, length, color):
        ...
        color_r = (color >> 8) & 0xF8
        color_g = (color >> 3) & 0xFC
        color_b = (color << 3)
        
        # 4. 批量生成数据：用bytearray预分配内存（比循环append快3倍）
        batch_data = bytearray(pixel_count * 3)
        for i in range(pixel_count):
            batch_data[i*3] = color_r
            batch_data[i*3 + 1] = color_g
            batch_data[i*3 + 2] = color_b
    ...
    def blit_buffer(self, buffer: bytearray, x: int, y: int, width: int, height: int):
        ...
        limit = min(len(buffer), width * height * 3)
        # 计算完整块数量和剩余字节数
    ...
    def map_bitarray_to_rgb565(self, bitarray, buffer, width, color, bg_color):
        ...
        for i in range(length):
            ...
            for bi in range(0, 8, 1):
                ...
                buffer[id] = (cur_color >> 8) & 0xF8 
                id += 1
                buffer[id] = (cur_color >> 3) & 0xFC      
                id += 1
                buffer[id] = (cur_color << 3)      
                id += 1                
                # 更新当前行的像素位置
                row_pos += 1
    ...
    def show_text(self, x: int, y: int, num: str, fc:int,bc:int,sizey:int):
        ...
        for sprite_bitmap in num:
            ...
            sprite = bytearray(typeface_num*8*3) #存储字符颜色
    ...
    def LCD_ShowPicture(self, filename, x: int, y: int, chunk_size=MAX_BUFFER_SIZE):    
        ...
        with open(filename, 'rb') as f:
            ...
            while bytes_read < total_data_size:
                ...
                rgb565_chunk = bytearray(len(chunk))
                id=0
                for i in range(0,len(chunk),3):
                    rgb565_chunk[id]=chunk[i+2]
                    id += 1
                    rgb565_chunk[id]=chunk[i+1]
                    id += 1
                    rgb565_chunk[id]=chunk[i]
                    id += 1
